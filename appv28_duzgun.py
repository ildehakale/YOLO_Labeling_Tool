...
class BatchGkdtWorker(QThread):
        ...
        try:
            for i, image_filename in enumerate(self.image_paths_to_process):
                ...
                try:
                    ...
                    # Process and filter results using the same logic as the single image button
                    processed_results = self.process_gkdt_results_internal(
                        gkdt_results,
                        image_size.width(),
                        image_size.height(),
                        image_filename 
    ...
    def process_gkdt_results_internal(self, gkdt_results, image_width, image_height, image_name):
        allowed_classes = {0, 2}
        unique_gkdt_boxes = []
        iou_threshold = 0.1 
        ...
        # Check against previously added GKST boxes from this batch.
        existing_gkdt_boxes = []
        for img_name, boxes in self.batch_results.items():
...
class ModernImageNavigator(QMainWindow):
    ...
    def save_gkdt_results(self):
        ...
        try:
            ...
            for image_name, gkdt_boxes in self.all_gkdt_boxes.items():
                ...
                with open(label_path, 'a') as f:
                    for cid, x, y, w, h in gkdt_boxes:
                        ...
                        line = f"{int(cid)} {nx:.6f} {ny:.6f} {nw:.6f} {nh:.6f}"
                        f.write(line + "\n")
                saved_count += len(gkdt_boxes)
            
            self.all_gkdt_boxes.clear()

            QMessageBox.information(self, "Başarılı", f"{saved_count} GKST sonucu kaydedildi.")
            ...
            QMessageBox.critical(self, "Hata", f"GKST sonuçları kaydedilirken bir hata oluştu: {e}")
    ...
    def on_batch_image_processed(self, image_name, processed_results):
        # Bu fonksiyon sadece görsel güncellemeyi yönetir.
        # all_gkdt_boxes'a ekleme işi zaten worker içinde yapılıyor.
        
        # Eğer güncellenen resim şu an ekranda olan resimse, anlık olarak etiketi çiz.

...
class StatsWidget(QWidget):
    ...
    def update_stats(self, total_images=0, current_image=0, selected=0, existing_labels=0, detector_added=0, total_labels=0):
        ...
        self.stat_labels["sahi_eklenen"].setText(str(detector_added))
...
class SAHIThread(QThread):
    finished = pyqtSignal(list)
    error = pyqtSignal(str)

    def __init__(self, image_path, detector_predictor, confidence, slice_height, slice_width, parent=None):
        super().__init__(parent)
        self.image_path = image_path
        self.detector_predictor = detector_predictor
    ...
    def run(self):
        try:
            if isinstance(self.detector_predictor, ONNXDetectionModel):
                self.detector_predictor.confidence_threshold = self.confidence
                result = self.detector_predictor.perform_inference(self.image_path)
            else:
                self.detector_predictor.confidence_threshold = self.confidence
            ...
            for detection in result.object_prediction_list:
                ...
                detector_results.append((class_id, bbox[0], bbox[1], bbox[2], bbox[3]))
                
            self.finished.emit(detector_results)
        except Exception as e:
            self.error.emit(str(e))
            self.finished.emit([]) 
            
class BatchdetectorWorker(QThread):
    ...
    def run(self):
        try:
            for i, image_filename in enumerate(self.image_paths_to_process):
                ...
                try:
                    # Raw SAHI results
                    if isinstance(self.parent_app.detector_predictor, ONNXDetectionModel):
                        self.parent_app.detector_predictor.confidence_threshold = self.confidence
                        result = self.parent_app.detector_predictor.perform_inference(current_image_path)
                    else:
                        self.parent_app.detector_predictor.confidence_threshold = self.confidence
                        result = get_sliced_prediction(
                            current_image_path,
                            self.parent_app.detector_predictor,
                            slice_height=self.slice_height,
                            slice_width=self.slice_width,
                            overlap_height_ratio=0.2,
                            overlap_width_ratio=0.2
                    ...
                    processed_results = self.parent_app.process_detector_results(
                        detector_results,
                        image_size.width(),
                        image_size.height(),
                        image_filename 
    ...
    def process_detector_results_for_batch(self, detector_results, image_width, image_height, image_name):
        """Tekli işlemle tamamen aynı mantık"""
        allowed_classes = {0, 2}
        unique_detector_boxes = []
        iou_threshold = 0.1 
        ...
        # Önceki SAHI sonuçlarını kontrol et
        existing_detector_boxes = []
        # Ana uygulamadaki all_detector_boxes'tan al
        if hasattr(self.parent_app, 'all_detector_boxes') and image_name in self.parent_app.all_detector_boxes:
            existing_detector_boxes = self.parent_app.all_detector_boxes[image_name]
        ...
        for processed_img_name, boxes in self.batch_results.items():
            if processed_img_name != image_name:  # Kendisi hariç
                existing_detector_boxes.extend(boxes)

        # Duplikasyon kontrolü - tekli işlemle aynı
        for class_id, x1, y1, x2, y2 in detector_results:
            if class_id in allowed_classes:
                w = x2 - x1
                h = y2 - y1
                
                is_duplicate = False
                # Hem mevcut etiketlerle hem de önceki SAHI sonuçlarıyla karşılaştır
                for existing_cid, ex, ey, ew, eh in existing_boxes + existing_detector_boxes:
                ...
                if not is_duplicate:
                    unique_detector_boxes.append((class_id, x1, y1, w, h))
        
        return unique_detector_boxes
...
class ModernImageNavigator(QMainWindow):
    def __init__(self):
        ...
        self.all_detector_boxes = {}
        self.detector_added_count = 0

        self.device = 'cuda:0' if SAHI_AVAILABLE and torch.cuda.is_available() else 'cpu'
    ...
    def load_detector_model(self):
        if not SAHI_AVAILABLE:
            self.detector_status_label.setText("SAHI ve/veya Ultralytics kütüphanesi eksik!")
        ...
        if not self.model_list.currentItem():
            self.detector_status_label.setText("Lütfen listeden bir model dosyası seçin.")
            self.detector_btn.setEnabled(False)
            self.batch_detector_btn.setEnabled(False)
            self.save_detector_btn.setEnabled(False)
            return
        ...
        if not os.path.exists(model_path):
            self.detector_status_label.setText(f"Hata: {model_path} dosyası bulunamadı!")
            self.detector_btn.setEnabled(False)
            self.batch_detector_btn.setEnabled(False)
            self.save_detector_btn.setEnabled(False)
            return
        ...
        try:
            self.detector_status_label.setText(f"SAHI Ultralytics modeli yükleniyor... ({self.device})")
            
            model_extension = model_path.lower().split('.')[-1]
            
            if model_extension == 'onnx':
                self.detector_predictor = ONNXDetectionModel(
                    model_path=model_path,
                    confidence_threshold=self.confidence_input.value(),
                    device=self.device
                )
                precision_info = getattr(self.detector_predictor, 'precision_info', 'FP32')
                runtime_info = "ONNX Runtime" if getattr(self.detector_predictor, 'use_onnx', False) else "Ultralytics"
                self.detector_status_label.setText(f"SAHI ONNX modeli hazır. Cihaz: {self.device} ({precision_info}) - {runtime_info}")
                
            elif model_extension == 'pt':
                self.detector_predictor = UltralyticsDetectionModel(
                    model_path=model_path,
                    confidence_threshold=self.confidence_input.value(),
                    device=self.device
                )
                ...
                if self.device.startswith('cuda'):
                    try:
                        self.detector_predictor.model.model.half()
                        self.detector_status_label.setText(f"SAHI PyTorch modeli hazır. Cihaz: {self.device} (FP16)")
                    except Exception as half_error:
                        self.detector_status_label.setText(f"SAHI PyTorch modeli hazır. Cihaz: {self.device} (FP32)")
                else:
                    self.detector_status_label.setText(f"SAHI PyTorch modeli hazır. Cihaz: {self.device} (FP32)")
            else:
            ...
            self.detector_status_label.setText(f"SAHI Ultralytics modeli yüklenemedi: {e}")
            self.detector_btn.setEnabled(False)
            self.batch_detector_btn.setEnabled(False)
            self.save_detector_btn.setEnabled(False)
    ...
    def load_default_models(self):
        ...
        if os.path.exists(self.model_folder):
            ...
            self.detector_status_label.setText("`models` klasörü bulunamadı. Lütfen bir model klasörü oluşturun ve içine model dosyalarınızı koyun.")
            self.detector_btn.setEnabled(False)
            self.batch_detector_btn.setEnabled(False)
            self.save_detector_btn.setEnabled(False)
    ...
    def update_model_list(self, folder):
        ...
        if model_files:
            ...
            self.detector_status_label.setText("Seçilen klasörde model dosyası (.pt veya .onnx) bulunamadı.")
            self.detector_btn.setEnabled(False)
            self.batch_detector_btn.setEnabled(False)
            self.save_detector_btn.setEnabled(False)

    def update_detector_confidence(self, value):
        if self.detector_predictor:
            if hasattr(self.detector_predictor, 'confidence_threshold'):
                self.detector_predictor.confidence_threshold = value
    ...
    def create_sidebar(self):
        ...
        detector_settings_layout = QHBoxLayout()
        detector_confidence_label = QLabel("SAHI Güvenilirlik:")
        detector_confidence_label.setFont(QFont("Segoe UI", 9))
        ...
        """)
        detector_settings_layout.addWidget(detector_confidence_label)
        detector_settings_layout.addWidget(self.confidence_input)
        tools_layout.addLayout(detector_settings_layout)
        ...
        self.detector_btn = ModernButton("✨ SAHI Uygula", "primary")
        self.detector_btn.setEnabled(False)
        tools_layout.addWidget(self.detector_btn)

        self.batch_detector_btn = ModernButton("✨ SAHI Uygula (50 Resim)", "primary")
        self.batch_detector_btn.setEnabled(False)
        tools_layout.addWidget(self.batch_detector_btn)

        self.save_detector_btn = ModernButton("💾 SAHI Sonuçlarını Kaydet", "secondary")
        self.save_detector_btn.setEnabled(False)
        tools_layout.addWidget(self.save_detector_btn)

        self.detector_status_label = QLabel(f"SAHI durumu: Cihaz: {self.device}")
        ...
        self.detector_btn.clicked.connect(self.apply_detector_detection)
        self.batch_detector_btn.clicked.connect(self.start_batch_detector_process)
        self.save_detector_btn.clicked.connect(self.save_detector_results)
        self.confidence_input.valueChanged.connect(self.update_detector_confidence)
        self.model_list.itemSelectionChanged.connect(self.load_detector_model)
    ...
    def process_detector_results(self, detector_results, image_width, image_height, image_name):
        allowed_classes = {0, 2,3,4,5}
        unique_detector_boxes = []
        iou_threshold = 0.1 
        ...
        existing_detector_boxes = self.all_detector_boxes.get(image_name, [])
    ...
    def apply_detector_detection(self):
        if not SAHI_AVAILABLE:
            QMessageBox.critical(self, "Hata", "SAHI veya Ultralytics kütüphanesi eksik.")
            return

        if not self.detector_predictor:
            QMessageBox.warning(self, "Uyarı", "SAHI modeli henüz hazır değil. Lütfen bekleyin.")
            return
        ...
        if self.detector_thread and self.detector_thread.isRunning():
            QMessageBox.warning(self, "Uyarı", "Başka bir SAHI işlemi zaten devam ediyor.")
            return

        self.detector_status_label.setText(f"SAHI algılama başlatılıyor... Cihaz: {self.device}")
        self.detector_btn.setEnabled(False)
        self.batch_detector_btn.setEnabled(False)
        self.save_detector_btn.setEnabled(False)
        ...
        self.detector_thread = SAHIThread(current_image_path, self.detector_predictor, confidence, slice_height, slice_width)
        self.detector_thread.finished.connect(self.on_detector_finished)
        self.detector_thread.error.connect(self.on_detector_error)
        self.detector_thread.start()

    def on_detector_finished(self, detector_results):
            self.detector_status_label.setText("SAHI algılama tamamlandı. Yeşil kutuları kaydetmek için butona basın.")
            self.detector_btn.setEnabled(True)
            self.batch_detector_btn.setEnabled(True)
            self.save_detector_btn.setEnabled(True)
            ...
            # Sadece tekil işlemde, ilgili resmin önceki SAHI kutularını temizle
            if current_image_name in self.all_detector_boxes:
                del self.all_detector_boxes[current_image_name]

            processed_results = self.process_detector_results(
                detector_results,
            ...
            # all_detector_boxes'a yeni işlenmiş sonuçları ekle
            self.all_detector_boxes[current_image_name] = processed_results
            self.detector_added_count = len(processed_results)
    ...
    def on_detector_error(self, message):
        self.detector_status_label.setText(f"SAHI algılama hatası: {message}")
        self.detector_btn.setEnabled(True)
        self.batch_detector_btn.setEnabled(True)
        self.save_detector_btn.setEnabled(False)
        QMessageBox.critical(self, "SAHI Algılama Hatası", message)
        self.detector_thread.quit()
        self.detector_thread.wait()

    def save_detector_results(self):
            print(f"save_detector_results çağrıldı")
            print(f"all_detector_boxes içeriği: {len(self.all_detector_boxes)} resim")
            print(f"labels_folder: {self.labels_folder}")
            
            if not self.all_detector_boxes:
                QMessageBox.information(self, "Bilgi", "Kaydedilecek SAHI sonucu bulunamadı.")
                return
            ...
            try:
                ...
                for image_name, detector_boxes in self.all_detector_boxes.items():
                    print(f"İşleniyor: {image_name}, {len(detector_boxes)} kutu")
                    ...
                    try:
                        with open(label_path, 'a', encoding='utf-8') as f:
                            for cid, x, y, w, h in detector_boxes:
                ...
                if saved_count > 0:
                    ...
                    self.all_detector_boxes.clear()
                    
                    QMessageBox.information(self, "Başarılı", f"{saved_count} SAHI sonucu YOLO formatında kaydedildi.")
                    
                    # detector eklenen sayacını sıfırla
                    self.detector_added_count = 0   
                    ...
                    QMessageBox.warning(self, "Uyarı", "Hiçbir SAHI sonucu kaydedilemedi.")
                
            except Exception as e:
                error_msg = f"SAHI sonuçları kaydedilirken bir hata oluştu: {e}"
    ...
    def start_batch_detector_process(self):
        if not self.detector_predictor:
            QMessageBox.warning(self, "Uyarı", "SAHI modeli henüz hazır değil. Lütfen bekleyin.")
            return
        ...
        reply = QMessageBox.question(
            ...
            f'Mevcut resimden başlayarak sonraki {num_images_to_process} resme toplu SAHI uygulamak istediğinizden emin misiniz? Sonuçlar geçici olarak saklanacaktır.',
            QMessageBox.Yes | QMessageBox.No
        )

        if reply == QMessageBox.Yes:
            self.detector_status_label.setText(f"Toplu işlem başlatılıyor... {num_images_to_process} resim işleniyor.")
            ...
            self.batch_worker = BatchdetectorWorker(self, images_to_process)
    ...
    def update_batch_progress(self, current, total):
        ...
        self.detector_status_label.setText(f"İşleniyor: Resim {current} / {total}")

    def on_batch_finished(self, batch_results):
        self.detector_status_label.setText("Toplu SAHI işlemi tamamlandı. Sonuçlar geçici belleğe aktarıldı.")
        
        # Tüm batch sonuçlarını all_detector_boxes'a ekle
        self.all_detector_boxes.update(batch_results)
        ...
        if current_image_name and current_image_name in self.all_detector_boxes:
            self.detector_added_count = len(self.all_detector_boxes[current_image_name])
        else:
            self.detector_added_count = 0
        ...
        QMessageBox.information(self, "Başarılı", "Toplu SAHI işlemi başarıyla tamamlandı. Artık resimler arasında gezinerek yeşil etiketleri görebilirsiniz.")
        self.detector_btn.setEnabled(True)
        self.batch_detector_btn.setEnabled(True)
        self.save_detector_btn.setEnabled(True)
    ...
    def on_batch_image_processed(self, image_name, processed_results):
        ...
        # all_detector_boxes'a hemen ekle
        if image_name not in self.all_detector_boxes:
            self.all_detector_boxes[image_name] = []
        self.all_detector_boxes[image_name] = processed_results
    ...
    def on_batch_error(self, message):
        self.detector_status_label.setText("Toplu işlem sırasında bir hata oluştu.")
    ...
    def update_stats(self):
        ...
        current_detector_boxes = self.all_detector_boxes.get(current_image_name, [])
        
        self.detector_added_count = len(current_detector_boxes)
        selected = len(self.selected) + len(self.selected_detector)
        existing_labels = len(self.boxes)
        
        total_labels = existing_labels + self.detector_added_count

        self.stats_widget.update_stats(total_images, current_image, selected, existing_labels, self.detector_added_count, total_labels)
    ...
    def update_class_filter_list(self):
        ...
        for detector_boxes in self.all_detector_boxes.values():
            for cid, _, _, _, _ in detector_boxes:
                unique_classes.add(int(cid))
    ...
    def on_mouse_press(self, event):
        if not self.scaled_pix.isNull():
            if event.button() == Qt.LeftButton:
                ...
                current_image_name = self.image_paths[self.index]
                boxes_to_check = self.all_detector_boxes.get(current_image_name, [])

                for idx, (_, x, y, w, h) in enumerate(boxes_to_check):
                    if x <= x_im <= x + w and y <= y_im <= y + h:
                        if idx in self.selected_detector:
                            self.selected_detector.remove(idx)
                        else:
                            self.selected_detector.add(idx)
                        detector_selected = True
                        break
                
                if not detector_selected:
            ...
            if rw > 10 and rh > 10:
                ...
                if dialog.exec_() == QDialog.Accepted:
                    ...
                    if class_id.isdigit():
                        ...
                        # YOLO formatında normalize et ve kaydet
                        img_w = self.original_image_size.width()
                        img_h = self.original_image_size.height()
                        
                        center_x = (ox + ow / 2) / img_w
                        center_y = (oy + oh / 2) / img_h
                        norm_width = ow / img_w
                        norm_height = oh / img_h
                        
                        yolo_line = f"{class_id_int} {center_x:.6f} {center_y:.6f} {norm_width:.6f} {norm_height:.6f}"
                        self.labels.append(yolo_line)
                        
                        # Hemen dosyaya kaydet
                        self.save_current_labels()
                        
                        # Sınıf filtresini güncelle
                        self.update_class_filter_list()
            
            # Çizim durumunu temizle ve ekranı güncelle
            self.start_pos = QPointF()
            self.end_pos = QPointF()
            self.update_image()
    ...
    def save_current_labels(self):
        """Mevcut etiketleri dosyaya kaydet"""
        if not self.image_paths or not self.labels_folder:
            return
        ...
        try:
            ...
            # Tüm etiketleri dosyaya yaz (overwrite mode)
            with open(label_path, 'w', encoding='utf-8') as f:
                for label in self.labels:
                    if label.strip():  # Boş satırları atla
                        f.write(label.strip() + "\n")
                        
            print(f"Etiketler kaydedildi: {label_path}")
            
        except Exception as e:
            print(f"Etiket kaydetme hatası: {e}")
            QMessageBox.warning(self, "Hata", f"Etiketler kaydedilemedi: {e}")
    ...
    def update_image(self):
        ...
        # Mevcut etiketleri çiz (kırmızı kutular)
        ...
        # SAHI kutularını çiz (yeşil kutular)
        current_image_name = self.image_paths[self.index] if self.image_paths and self.index < len(self.image_paths) else None
        detector_boxes_to_draw = self.all_detector_boxes.get(current_image_name, [])

        for idx, (cid, x, y, w, h) in enumerate(detector_boxes_to_draw):
            if int(cid) in visible_classes:
                class_name = self.class_names.get(cid, f"C{cid}")
            
                if idx in self.selected_detector:
                    detector_pen = QPen(QColor(255, 193, 7), 3)
                else:
                    detector_pen = QPen(QColor(16, 185, 129), 2)
            
                painter.setPen(detector_pen)
                r = QRectF(x*self.sf_w, y*self.sf_h, w*self.sf_w, h*self.sf_h)
                painter.drawRect(r)

                detector_label_x = int(x * self.sf_w)
                detector_label_y = max(0, int(y * self.sf_h - 20))

                font_metrics = painter.fontMetrics()
                label_width = font_metrics.width(class_name) + 10
                label_height = font_metrics.height() + 4
            
                bg_color = QColor(255, 193, 7) if idx in self.selected_detector else QColor(16, 185, 129)
                painter.fillRect(
                    detector_label_x, detector_label_y, label_width, label_height, bg_color
                )
                painter.setPen(QPen(Qt.white))
                painter.drawText(
                    detector_label_x + 5, detector_label_y + 12, class_name
                )

        # Çizim önizlemesi (çizim sırasında görünen mavi kesikli çizgi)
        if self.is_drawing and hasattr(self, 'start_pos') and hasattr(self, 'end_pos'):
            preview = QPen(QColor(66, 153, 225), 2, Qt.DashLine)
    ...
    def delete_selected_detector_boxes(self):
        if not self.selected_detector:
            return

        current_image_name = self.image_paths[self.index]
        new_detector_boxes = [
            box for i, box in enumerate(self.all_detector_boxes.get(current_image_name, []))
            if i not in self.selected_detector
        ]
        self.all_detector_boxes[current_image_name] = new_detector_boxes

        self.selected_detector.clear()
        self.update_image()
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Right:
            ...
            self.delete_selected_detector_boxes()
...
def main():
    ...
    app.setApplicationName("Modern LABEL Etiketleme Aracı")
